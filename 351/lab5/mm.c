...
//    char* implicitly.
static inline void *UNSCALED_POINTER_ADD(void *p, int x) {
        return ((void *)((char *)(p) + (x)));
}
static inline void *UNSCALED_POINTER_SUB(void *p, int x) {
        return ((void *)((char *)(p) - (x)));
}
...
struct block_info {
        ...
        // together. See the SIZE() function and TAG macros below for more
        // details and how to extract these pieces of info.
...
static void examine_heap() {
        ...
        for (block = (block_info *)UNSCALED_POINTER_ADD(
                 mem_heap_lo(), WORD_SIZE); // first block on heap
             SIZE(block->size_and_tags) != 0 &&
             block < (block_info *)mem_heap_hi();
             block = (block_info *)UNSCALED_POINTER_ADD(
                 block, SIZE(block->size_and_tags))) {
                ...
                if (block->size_and_tags & TAG_USED) {
                        ...
                        fprintf(stderr, "FREE\tnext: %p, prev: %p\n",
                                (void *)block->next, (void *)block->prev);
                }
        }
...
static void coalesce_free_block(block_info *old_block) {
        ...
        while ((block_cursor->size_and_tags & TAG_PRECEDING_USED) == 0) {
                ...
                size_t size = SIZE(
                    *((size_t *)UNSCALED_POINTER_SUB(block_cursor, WORD_SIZE)));
                // Use this size to find the block info for that block.
                free_block =
                    (block_info *)UNSCALED_POINTER_SUB(block_cursor, size);
        ...
        while ((block_cursor->size_and_tags & TAG_USED) == 0) {
                ...
                new_size += size;
                block_cursor =
                    (block_info *)UNSCALED_POINTER_ADD(block_cursor, size);
        }
        ...
        if (new_size != old_size) {
                ...
                // The boundary tag of the preceding block is the word
                // immediately preceding block in memory where we left off
                // advancing block_cursor.
                *(size_t *)UNSCALED_POINTER_SUB(block_cursor, WORD_SIZE) =
                    new_size | TAG_PRECEDING_USED;
...
static void request_more_space(size_t req_size) {
        ...
        new_block =
            (block_info *)UNSCALED_POINTER_SUB(mem_sbrk_result, WORD_SIZE);
        ...
        ((block_info *)UNSCALED_POINTER_ADD(new_block, total_size - WORD_SIZE))
            ->size_and_tags = total_size | prev_last_word_mask;

        // Initialize new end-of-heap word: SIZE is 0, TAG_PRECEDING_USED is 0,
        // TAG_USED is 1. This trick lets us do the "normal" check even at the
        // end of the heap.
...
int mm_init() {
        ...
        // of free list), MIN_BLOCK_SIZE bytes of space, WORD_SIZE byte
        // heap-footer.
        ...
        first_free_block =
            (block_info *)UNSCALED_POINTER_ADD(mem_heap_lo(), WORD_SIZE);

        // Total usable size is full size minus heap-header and heap-footer
        // words. NOTE: These are different than the "header" and "footer" of a
        // block!
        //  - The heap-header is a pointer to the first free block in the free
        //  list.
        //  - The heap-footer is the end-of-heap indicator (used block with size
        //  0).
        ...
        *((size_t *)UNSCALED_POINTER_ADD(first_free_block,
...
void *mm_malloc(size_t size) {
        ...
        // returned allocated address will be offset by the size of the header
