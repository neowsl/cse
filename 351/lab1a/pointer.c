...
int within_same_block(int *ptr1, int *ptr2) {
        // first cast to longs to perform pointer arithmetic over bytes
        long ptr1_value = (long)ptr1;
        long ptr2_value = (long)ptr2;

        // 64 = 2^6
        // right-shift each long by 6 places to "floor divide" by 64
        return ptr1_value >> 6 == ptr2_value >> 6;
}
...
int within_array(int *int_array, int size, int *ptr) {
        long start_ptr_value = (long)int_array;
        // can only go up to `size - 1` bits
        long end_ptr_value = (long)(int_array + size) - 1;
        long ptr_value = (long)ptr;

        // the *signs* of the differences between the start/end pointer values
        // and the actual pointer should be different
        // to get the sign of a signed value, right-shift by `# of bits - 1`
        return !((start_ptr_value - ptr_value) >> 63 ==
                 (end_ptr_value - ptr_value) >> 63);
}
...
int endian_experiment(int *ptr) {
        ...
        // 351351 = 0x00055C77
        // also need to remember to set leading zeroes!
        // convert to little endian: 77 | 5C | 05 | 00
        *(byte_ptr + 0) = 0x77;
        *(byte_ptr + 1) = 0x5C;
        *(byte_ptr + 2) = 0x05;
        *(byte_ptr + 3) = 0x00;
